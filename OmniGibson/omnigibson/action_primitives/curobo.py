...
# 大幅放宽碰撞检测限制以提高规划成功率
m.DEFAULT_COLLISION_ACTIVATION_DISTANCE = 0.025  # 大幅放宽到 0.025 (原始的5倍)
m.DEFAULT_ATTACHED_OBJECT_SCALE = 0.4  # 进一步缩小附着物体到 0.4
...
class CuRoboMotionGenerator:
    ...
    def _attach_objects_to_robot(
        ...
        for ee_link_name, obj in attached_obj.items():
            ...
            try:
                # Use SAMPLE_SURFACE instead of VOXEL_VOLUME_SAMPLE_SURFACE to avoid segfault
                self.mg[emb_sel].attach_objects_to_robot(
                    ...
                    surface_sphere_radius=0.002 * scale,
                    sphere_fit_type=lazy.curobo.geom.sphere_fit.SphereFitType.SAMPLE_SURFACE,
                )
                # Only add to attached_info if successful
                attached_info.append(
                    {"obj_paths": obj_paths, "link_name": self.robot.curobo_attached_object_link_names[ee_link_name]}
                )
            except Exception as e:
                # If attach fails, log warning but don't add to attached_info
                print(f"Warning: Failed to attach object {obj_paths} to robot: {e}")
